...
export default function BarcodeInput({ onScan, disabled, initialMode = "scanner" }: BarcodeInputProps) {
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      {/* Mode toggle */}
      <div className="flex rounded-2xl overflow-hidden border border-zinc-200 dark:border-zinc-700 w-full">
        <button
          type="button"
          onClick={() => setMode("scanner")}
          disabled={disabled}
          className={`flex-1 flex items-center justify-center gap-2 py-3 text-sm font-medium transition-colors min-h-[48px] ${
            mode === "scanner"
      ...
      {mode === "scanner" && (
        <div className="relative">
          ...
          />
          <div
            className="flex flex-col items-center justify-center gap-2 rounded-2xl border-2 border-dashed border-zinc-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 p-6 text-center cursor-pointer min-h-[120px]"
          ...
          >
            ...
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Ready to scan — point your barcode scanner at a product
            </p>
            {lastBarcode && (
      ...
      {mode === "camera" && (
        <div className="relative rounded-2xl overflow-hidden bg-black aspect-video w-full">
          ...
          {showOverlay && (
            <div className="absolute inset-0 border-4 border-green-400 rounded-2xl pointer-events-none animate-pulse" />
          )}
